...
| 2.4f | **`switch_certificate_issuer()`:** Automatisierter Wechsel des Ausstellers für alle Ingress-Ressourcen. Setzt staging auf ALLE Ingresses, wartet auf READY, prüft Aussteller auf `(STAGING)`, setzt dann production auf ALLE Ingresses, prüft erneut. Report mit Erfolg/Fehler pro Host, kein Abbruch bei Einzelfehlern. | `kubectl get ingress -n cc-prd-access-stack -o jsonpath='{.items[*].metadata.annotations.cert-manager\.io/cluster-issuer}'` – alle Einträge müssen `letsencrypt-prod` lauten |
