  ...
  <div class="quality-status" :class="`quality-${qualityClass}`">
    <div class="status-header">
      <span class="status-icon">
        {{ qualityClass === 'sehr-gut' ? '✅' : qualityClass === 'gut' ? '🔵' : qualityClass === 'ausreichend' ? '🟡' : '🔴' }}
      </span>
    ...
    <div class="status-details">
      <div class="metric">
        ...
        <div class="progress-bar">
          <div class="progress-fill" :style="{ width: `${props.quality.completeness}%` }"></div>
        </div>
        <span class="metric-value">{{ props.quality.completeness }}%</span>
      </div>

      <div class="metric">
        <span class="metric-label">Korrektheit:</span>
        <div class="progress-bar">
          <div class="progress-fill" :style="{ width: `${props.quality.accuracy}%` }"></div>
        </div>
        <span class="metric-value">{{ props.quality.accuracy }}%</span>
      </div>

      <div v-if="props.quality.reviewed && props.quality.reviewer" class="review-info">
        ✓ Geprüft von <strong>{{ props.quality.reviewer }}</strong> am {{ props.quality.reviewDate }}
      </div>
      <div v-else class="review-info pending">
        ⏳ Noch nicht geprüft
      </div>
    </div>
  </div>
