    ...
    <div class="quality-status" :class="statusClass">
        ...
        <div class="quality-metrics">
            ...
            <div class="metric">
                ...
                <div
                    class="review-status"
                    :class="quality.reviewed ? 'reviewed' : 'pending'"
                >
                    {{ quality.reviewed ? "✅ Geprüft" : "⏳ Ausstehend" }}
                    <span
                        v-if="quality.reviewed && quality.reviewDate"
                        class="review-date"
                    >
...
const overallScore = computed(() => {
    return Math.round(
        (props.quality.completeness + props.quality.accuracy) / 2,
    );
});

const overallClass = computed(() => {
    if (overallScore.value >= 80) return "sehr-gut";
    if (overallScore.value >= 60) return "gut";
    if (overallScore.value >= 40) return "ausreichend";
