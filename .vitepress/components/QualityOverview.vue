    ...
    <div class="quality-filters">
      ...
      <div class="filter-group">
        ...
        <select v-model="selectedQuality" @change="filterDocuments">
          ...
          <option value="excellent">Exzellent (80-100%)</option>
          <option value="good">Gut (60-79%)</option>
          <option value="fair">Ausreichend (40-59%)</option>
          <option value="poor">Verbesserungsbedarf (0-39%)</option>
        </select>
      </div>
    </div>

    <div class="documents-list">
      <div v-for="doc in filteredDocuments" :key="doc.path" class="document-item">
        ...
        <div class="document-quality">
          ...
          <div class="quality-details">
            <div class="detail">
              ...
              <span class="detail-value">{{ doc.quality.completeness }}%</span>
            </div>
            <div class="detail">
              <span class="detail-label">Genauigkeit:</span>
              <span class="detail-value">{{ doc.quality.accuracy }}%</span>
            </div>
            <div class="detail">
              <span class="detail-label">Review:</span>
              <span
                class="review-badge"
                :class="doc.quality.reviewed ? 'reviewed' : 'pending'"
              >
...
const filteredDocuments = computed(() => {
  ...
  if (selectedStatus.value !== 'all') {
    filtered = filtered.filter(doc =>
      selectedStatus.value === 'reviewed' ? doc.quality.reviewed : !doc.quality.reviewed
    )
  }

  if (selectedQuality.value !== 'all') {
    filtered = filtered.filter(doc => doc.qualityClass === selectedQuality.value)
  }
...
const averageScore = computed(() => {
  ...
  const total = documents.value.reduce((sum, doc) => sum + doc.overallScore, 0)
...
const qualityRanges = computed(() => {
  const ranges = [
    { label: 'Exzellent', min: 80, max: 100, class: 'excellent' },
    { label: 'Gut', min: 60, max: 79, class: 'good' },
    { label: 'Ausreichend', min: 40, max: 59, class: 'fair' },
    { label: 'Verbesserungsbedarf', min: 0, max: 39, class: 'poor' }
  ]
  ...
  return ranges.map(range => {
    ...
    const percentage = totalDocuments.value > 0
